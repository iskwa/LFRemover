...
namespace LFRemover
{
    ...
    public partial class App : Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            this._dummyWindow = new DummyWindow();
        }
    ...
    public class DummyWindow : Window
    {
        ...
        public DummyWindow()
        {
            this.hotKey = new HotKeyHelper(this);
            this.hotKey.Register(ModifierKeys.Control | ModifierKeys.Shift, Key.R,
                OnEventRemoveLineFeed);
        }

        private void OnEventRemoveLineFeed(object sender, EventArgs args)
        {
            if (!Clipboard.ContainsText()) { return; }

            var text = Clipboard.GetText();

            string removedText = RemoveLineBreak(text);
        ...
        private string RemoveLineBreak(string text)
        {
            ...
            foreach (var line in lines)
            {
                ...
                string previousLine = list.Last();

                // 前の行が空行
                if (string.IsNullOrEmpty(previousLine))
                {
                    // 空行以外（空白のみで構成される文字列も含む）
                ...
                {
                    // TODO 先頭に空白があれば改行する、とするか
                    if (line.Length > 0 && char.IsLower(line[0]))
                    {
                        list.RemoveAt(list.Count - 1);
                        list.Add(previousLine + " " + line);
                    }
                    else
                    {
                        list.Add(line.TrimEnd());
                ...
                if (previousLine.EndsWith("-"))
                {
                    if (!String.IsNullOrWhiteSpace(line))
                    {
                        ...
                        {
                            list.Add(previousLine.TrimEnd('-') + line);
                        }
                        else
                        {
                            list.Add(previousLine + line);
